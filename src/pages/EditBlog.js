...
function EditBlog(){
    ...
    const [contentFields, setContentFields] = useState([{header:'', body: '', footer: ''}]);
    ...
    const handelSubmitOnClick = (e) =>{
        e.preventDefault();
        let contentFieldsCopy = contentFields.filter(el => el.header !== '' || el.body !== '' || el.footer !== '');
        axios.put(`http://localhost:8080/api/v1/blogs/${blog.id}`, {
            ...formData,
            contents: contentFieldsCopy
        }).then(response =>{
           navigate("/admin")
        }).catch(error =>{
            console.log(error);
    ...
    useEffect(() =>{
        axios.get(`http://localhost:8080/api/v1/blogs/${blogId}`).then(response =>{
            ...
            setContentFields(response.data.contents);
            setFormData({
                des: response.data?.des,
                title: response.data?.title,
                linkImg: response.data?.linkImg,
            ...
            setContentNumber(response.data.contents.length);
            setSelectedOption(response.data.category.id);
        })
    },[blogId])
    return(
